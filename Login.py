...
if st.session_state["authentication_status"]:
    ...
    div[data-testid="stSidebarNav"].st-emotion-cache-79elbk.eczjsme17{
    display:none;
    }
    ...
    button.st-emotion-cache-1ny7cjd.ef3psqc12{
    position: fixed;
    right: 91%;
    top: 9%;
    }
            ...
            if user_input:
                ...
                with st.spinner("Searching for answer..."):
                    ...
                    if res.content == "2" or res.content == "Answer: 2":
                        try:
                            conn = psycopg2.connect(
                                host="65.20.77.132", user="master", password="0r5VB[TL?>A/8,}<vkpmEwS)",
                                database="ems_ai",
                                port="32432",
                            )
                            llm = ChatGroq(temperature=0.1,
                            ...
                            res1 = agent_executor.invoke(
                                {"input": "Generate sql query for this question : " + user_input})
                            ...
                            clr = ["#FF0000", "#0000FF", "#aaaaaa", "#00e5ff", "#00ff08", "#008000", "#ff009e",
                                   "#FFFF00"]
    ...
    Remember, your goal is to provide clear and accurate answers based on the user's query by interacting with the SQL database effectively.
            ...
            if f"generated2{index1}" in st.session_state:
