...
pg_uri = "postgresql+psycopg2://user1:z^{mG:*/tA+vRtRU;PG+_cc<@65.20.77.132:32433/postgres"
    ...
    if Main == False:
        ...
        # -----------------------------------------------Chat (CSV)-------------------------------------------------
            ...
            if f"past2{index1}" and f"generated2{index1}" not in st.session_state:
                ...
                st.markdown("""<table style="border: hidden;
                                    margin-top: 15%;
                                    position: fixed;
                                    width: 82%;
                                    ...
                                    height : 90px;
                                ">
                                  <tr style="border:hidden;height:75px;">
                                    <td style="border:hidden;"><button id="btn" style="border:1px solid;width:103%;height:105%;border-radius:10px;background:white;padding:inherite;">Explain all data</button></td>
                                    <td style="border:hidden;"><button id="btn" style="border:1px solid;width:103%;height:105%;border-radius:10px;background:white;padding:inherite;">Plot a chart for voltage vs time</button></td>
                                  </tr>
                                  <tr style="border:hidden;height:75px;">
                                    <td style="border:hidden;"><button id="btn"style="border:1px solid;width:103%;height:105%;border-radius:10px;background:white;padding:inherite;">How does current changes with time?</button></td>
                                    <td style="border:hidden;"><button id="btn" style="border:1px solid;width:103%;height:105%;border-radius:10px;background:white;padding:inherite;">What is the average energy consumption?</button></td>
                                  </tr>
                                </table>
                    ...
                    with st.spinner("Searching for answer..."):
                        ...
                        if res.content == "1":
                            ...
                            system_prefix1 = """You're an expert agent with exceptional prowess in SQL database interactions and data analysis.
                                                   ...
                                                   "voltage_r_15" : represents voltage from R phases, which have voltage of every 15 minutes. considering bucket (timestamp), avg (average), min (minimum) , max (maximum) of every 15 minutes.
    
                                                   "voltage_y_15" : represents voltage from Y phases, which have voltage of every 15 minutes. considering bucket (timestamp), avg (average), min (minimum) , max (maximum) of every 15 minutes.
    
                                                   "voltage_rb_15" : represents voltage from RB phases, which have voltage of every 15 minutes. considering bucket (timestamp), avg (average), min (minimum) , max (maximum) of every 15 minutes.
    
                                                   "voltage_ry_15" : represents voltage from RY phases, which have voltage of every 15 minutes. considering bucket (timestamp), avg (average), min (minimum) , max (maximum) of every 15 minutes.
    
                                                   "voltage_yb_15" : represents voltage from YB phases, which have voltage of every 15 minutes. considering bucket (timestamp), avg (average), min (minimum) , max (maximum) of every 15 minutes.
                                                   ...
                                                   use aggregates when user ask for current, voltage or kwh.
                        ...
                        if res.content == "3" or res.content == "Answer: 3":
                            ...
                            L1, L2, L3: Float type columns representing the current readings for the three separately phases .
    
                            Ln: Float type column representing the current of all phases combined in this.
                            ...
                            PF = (True power)/(Apparent power) here true power is (KT_R, KT_B, KT_Y, KT_TOTAL) column and apparent power is (KA_Y, KA_B, KA_R, KA_TOTAL) column. For example If you take KA_B as apparent power then consider KT_B true power.
                            Multiplying the voltage and current gives you the “apparent power”. This is measured in volt-amps (VA) rather than watts (W).
                            Multiplying this by the power factor gives you the “true power”.
                            The true power represents the real work that the electricity is doing.
